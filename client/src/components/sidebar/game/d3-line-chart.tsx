...
export const D3LineChart: React.FC<LineChartProps> = ({ data, width, height, margin }) => {
    ...
    useEffect(() => {
        ...
        const xScale = d3
            .scaleLinear()
            .domain([1, d3.max(data, (d) => d.round)!])
        ...
        function pointerMoved(event: MouseEvent) {
            ...
            tooltip.attr(
                'transform',
                `translate(${xScale(data[i].round)},${yScale(Math.max(data[i].brown, data[i].white))})`
            )
            ...
            const text = tooltip
                ...
                .call((text) =>
                    text
                        .selectAll('tspan')
                        .data(['Round: ' + data[i].round, 'Brown: ' + data[i].brown, 'White: ' + data[i].white])
                        .join('tspan')
                        .attr('x', 0)
                        .attr('y', (_, i) => `${i * 1.1}em`)
            ...
            const dataPointX = xScale(data[i].round)
            tooltipLine
                .attr('x1', dataPointX)
                .attr('y1', margin.top)
                .attr('x2', dataPointX)
